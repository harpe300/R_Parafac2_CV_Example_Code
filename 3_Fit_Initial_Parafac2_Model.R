...
# Fit a four-mode extension of Harshman's Parafac2 model (Helwig, Hong, & Bokhari, 2012) to the 4-way array to parsimoniously reduce dimensionality ...
# by explaining the maximum amount of variance across all four dimensions simultaneously. Contrast this with PCA/ICA/etc. reduction methods that only ...
# work on 2-way data and require the researcher to either 'throw out' portions of the data or unfold a multiway array into a two-dimensional matrix, ... 
# which causes significant interpretational difficulties. Unlike PCA/ICA/etc., Parafac2 allows for variation across a mode, meaning that the model ... 
# can flexibly capture and represent important between-variable differences in the component loadings (e.g., regional variation in sales, ... 
# between-participant latency shifts in a time-series vector, etc.)
 
# Use 10 random starts for each model, choosing the one with the lowest SSE as best-fitting. 
# Optimizes the chances that the chosen solution has within-sample uniqueness and the optimality of an obtained solution, rather than a solution that was caught in highly suboptimal local minima during optimization.
